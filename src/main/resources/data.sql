...
-- USERS init
-- pw must be a hash
insert into songmateUsers (username, password, is_account_non_locked, is_account_non_expired, enabled, is_credentials_non_expired)
...
CREATE TABLE authorities (
  ...
  FOREIGN KEY (username) REFERENCES songmateUsers(username)
);
...
-- Artist init
-- insert into songmateArtist (name, href, spotify_id, uri) values ('Kaz Bałagane', 'https://pl.wikipedia.org/wiki/Kaz_Ba%C5%82agane','1980', 'https://www.youtube.com/watch?v=SwOXYIEVf0w');
-- insert into songmateArtist (name, href, spotify_id, uri) values ('DDB', 'https://en.wikipedia.org/wiki/IAMDDB','1410', 'https://www.youtube.com/watch?v=pDgoDQn-6cs');
